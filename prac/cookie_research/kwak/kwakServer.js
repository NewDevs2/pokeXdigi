...
const server = http.createServer((req, rep) => {
  try {
    if (req.method === "GET") {
      ...
      if (req.url === "/checkcheckLogin") {
        console.log(req.headers.cookie);
        if (req.headers.cookie) {
          rep.writeHead(200, {"Content-Type":"text/json"});
          rep.write(JSON.stringify({uid:`${req.headers.cookie.split("=")[1]}`}));
          rep.end();
        } else {
          rep.end();
        }
          ...
          sign_master.query(
            ...
            function (err, result, fields) {
              ...
              if (result.length === 0) {
                ...
                //! 쿠키 연구하면서 수정한 부분 =========================================
                rep.writeHead(200, { "Content-Type": "text/html", "Set-Cookie":[`uid=${parsedData.UserID}; httpOnly`] });
                //! 쿠키 연구하면서 수정한 부분 =========================================
