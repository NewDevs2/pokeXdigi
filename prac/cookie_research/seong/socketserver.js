...
const server = http.createServer((req, res) => {
  if (req.method === 'GET') {
    if (req.url === '/') {
      ...
      res.write(page)
      res.end()
    } else if (req.url.includes('/enter')) {
      res.writeHead(200, { 'Content-Type': 'text/JSON' })
      res.write(JSON.stringify(decodeURIComponent(req.headers.cookie)))
      res.end()
    }
  }
  if (req.method === 'POST') {
    if (req.url.includes('/chatting')) {
      ...
      req.on('end', () => {
        ...
        // let usercookie = qs.parse(decodeURIComponent(req.headers.cookie))
